(()=>{
    var findWords = function(board, words) {
        ...
        class Node{
            ...
            has(chr){
                return chr in this.children
            }
            get(chr){
                return this.has(chr)?this.children[chr]:null
            }
            remove(chr){
                if (this.has(chr)){
            ...
            addWord(word){
                ...
                for (let l = 0; l < word.length; l++){
                    node = node.add(word[l])
                }
                node.word = word
            }
            add(chr){
                if (this.has(chr))return this.children[chr]
                this.children.length++
                return this.children[chr] = new Node()
            }
        }

        let traverse = (row, col, node) => {
            let chr = board[row][col]
            let currNode = node.get(chr)
            ...
            for (let i = 0; i < offsets.length; i++) {
                let nextR = row + offsets[i][0], nextC = col + offsets[i][1]
                if (!(nextR in board) || !(nextC in board[row])){
                    continue
                }
                if (currNode.has(board[nextR][nextC])) {
                    traverse(nextR, nextC, currNode)
                }
            }
            board[row][col] = chr
